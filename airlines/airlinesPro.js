const flights = [
  ...
  const airlineInterface = () => { 
    const userName = prompt("Bienvenid@ a ISDI Coders Airlines!\nPor favor, introduce tu nombre.");
    if (userName === '' || userName === ' ' ) {
        ...
        availableFlights();
        const typeOfUser = confirm(`Encantados de conocerte, ${userName.charAt(0).toUpperCase() + userName.slice(1)}!\nPara ADMIN pulsa OK, para USER pulsa CANCEL.`);
...
const admin = () => {   
    const adminAnswer = prompt('En calidad de ADMIN puedes añadir o eliminar vuelos.\nIntroduce 1 para añadir un vuelo.\nIntroduce 2 para eliminar un vuelo. ')
    if(adminAnswer === '1') {
        addFlights();
        return
        ...
        alert ('El valor introducido no es ni 1 ni 2.\nIntroduce 1 para añadir , 2 para eliminar.');
        admin();
    };
...
const addFlights = () => {
   ...
   if(flights.length === 15) {
    ...
    do {
        addNewFlight.to = prompt('* Obligatorio rellenar todos los campos!\nEscribe ORIGEN del vuelo');
    ...
    addNewFlight.scale = confirm('Pulse OK para vuelo con ESCALA\nPulse CANCEL para vuelo SIN ESCALA');
...
const deleteFlights = () => {
    ...
    for (let i = 0 ; i < flights.length; i++) {
        console.log(`Vuelo con id ${flights[i].id}, de ${flights[i].from}, y destino ${flights[i].to} coste ${flights[i].cost}€ `);
    }
    const idToDelete = prompt('En consola se muestra lista de vuelos.\n Escribe id para eliminar');
...
const searchFlightByPrice = userPrice => {
    ...
    if(filterPrice.length === 0){
        ...
        console.log(`Los vuelos con un precio igual o inferior a ${userPrice} son:\n`);
        for(let i = 0; i < filterPrice.length; i++) {
            console.log(`El vuelo con origen: ${filterPrice[i].from}, y destino ${filterPrice[i].to} tiene un coste de ${filterPrice[i].cost}€.`);
        };
    }; 
